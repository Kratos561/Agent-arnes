...
export const DEFAULT_RULES: AgentRule[] = [
  ...
  {
    id: 'rule-safety',
    name: 'Seguridad y Limites',
    content: 'No generes codigo malicioso, no accedas a informacion sensible sin autorizacion, no ejecutes acciones destructivas sin confirmacion. Si una solicitud viola principios eticos o de seguridad, explica por que y ofrece alternativas constructivas.',
    enabled: true,
    scope: 'global',
    category: 'safety',
    createdAt: Date.now(),
  ...
  {
    id: 'rule-skill-check',
    name: 'Revision de Skills y Herramientas',
    content: 'ANTES de cada tarea: (1) Revisa si hay skills activos que apliquen a la tarea. Si los hay, siguelas. (2) Si no hay skills, evalua que herramientas necesitas (web_search para datos actuales, render_chart para visualizaciones, generate_csv para datos tabulares). (3) Usa las herramientas mediante bloques :::tool con el formato correcto. (4) Si la tarea requiere crear un skill permanente, genera un SKILL.md con frontmatter YAML y instrucciones completas.',
    enabled: true,
    scope: 'global',
    category: 'behavior',
    createdAt: Date.now(),
...
export function assemblePrompt(ctx: AssembleContext): string {
  ...
  sections.push({
    ...
    order: 150,
    text: [
      '## Herramientas Disponibles — Sintaxis EXACTA',
      '',
      'Tienes acceso a herramientas reales que se ejecutan automaticamente. Para usarlas, emite UN BLOQUE por herramienta con este formato EXACTO (sin espacios extra antes de :::):',
      '',
      '```',
      ':::tool',
      ...
      '',
      '1. **web_search** — Busca en internet en tiempo real (DuckDuckGo + Wikipedia).',
      '   Arguments: `{"query":"termino de busqueda"}`',
      '   Ejemplo:',
      '   ```',
      '   :::tool',
      '   {"name":"web_search","arguments":{"query":"DeepSeek IA modelos 2026"}}',
      '   :::',
      '   ```',
      '',
      '2. **render_chart** — Renderiza una grafica en el chat (canvas, sin dependencias).',
      '   Arguments: `{"type":"bar|line|pie","labels":["Ene","Feb","Mar"],"datasets":[{"label":"Serie 1","data":[10,20,30]}],"title":"Titulo grafica"}`',
      '   Tipos: bar, line, pie, doughnut',
      '   Ejemplo:',
      '   ```',
      '   :::tool',
      '   {"name":"render_chart","arguments":{"type":"bar","labels":["Q1","Q2","Q3","Q4"],"datasets":[{"label":"Ventas","data":[150,230,180,310]}],"title":"Ventas trimestrales"}}',
      '   :::',
      '   ```',
      '',
      '3. **generate_csv** — Genera un archivo CSV descargable.',
      '   Arguments: `{"data":[{"col1":"val1","col2":"val2"}]}`',
      '',
      '### REGLAS CRITICAS para herramientas:',
      '- SIEMPRE usa el formato :::tool ... ::: para llamar herramientas.',
      '- NO uses "Tool: web_search query: ..." ni ningun otro formato.',
      '- Puedes emitir VARIOS bloques :::tool en una misma respuesta.',
      '- Despues de emitir el bloque, continua con tu respuesta normal.',
      '- Los resultados de web_search se insertan automaticamente en el chat.',
      '- Las graficas se renderizan automaticamente como imagenes.',
      '',
      '### Flujo para tareas que requieren informacion:',
      '1. Analiza que informacion necesitas.',
      '2. Si necesitas datos actuales, busca en la web con web_search.',
      '3. Si necesitas mostrar datos, usa render_chart.',
      '4. Si necesitas generar un documento, crea el contenido y luego pide al usuario que lo descargue.',
      '',
      '### Generacion de documentos:',
      'Cuando el usuario pida un documento (PDF, Excel, Word, etc.):',
      '- Genera el contenido completo en markdown o HTML.',
      '- Luego ofrece las opciones de descarga usando la herramienta de exportacion del chat.',
      '- NO uses bloques :::tool para documentos — el usuario puede exportar desde los botones.',
      '',
      '### Skills del agente:',
      'Antes de cada tarea, revisa si hay skills activos que apliquen. Si los hay, siguelas. Si no hay skills para la tarea, ejecutala con las herramientas disponibles y tu conocimiento.',
    ].join('\n'),
  });
