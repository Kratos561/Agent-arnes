...
export async function GET(req: NextRequest) {
  ...
  const result = await executeToolCall('backups_list', {}, {
    agent: auth.agent,
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const requestId = req.headers.get('x-request-id') || `req_${crypto.randomBytes(6).toString('hex')}`;
    const result = await executeToolCall('backups_rollback', body, {
      agent: auth.agent,
      request_id: requestId,
      dry_run: false,
    });
    ...
    if (!result.success) {
      return NextResponse.json({ error: result.message || 'Error en la restauración' }, { status: 400 });
    }
