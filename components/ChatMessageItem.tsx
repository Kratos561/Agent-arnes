  ...
  return (
    ...
    >
      <div className={`max-w-3xl mx-auto ${isUser ? 'flex justify-end' : 'flex items-start gap-3 sm:gap-4'}`}>
        ...
        {/* Content */}
        <div className={`${isUser ? 'max-w-[82%] sm:max-w-[525px]' : 'flex-1 min-w-0'} space-y-1.5`}>
          {/* Header */}
          {!isUser && (
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                {message.model && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded-md bg-accent-soft text-accent font-mono font-medium border border-accent/20">
                    {message.model}
                  </span>
                )}
              </div>
          ...
          {isUser && (
            <div className="text-[11px] text-neutral-400 dark:text-neutral-500 flex items-center gap-2 justify-end">
              <span className="font-semibold text-xs text-neutral-600 dark:text-neutral-300">Tu</span>
              <span suppressHydrationWarning>{message.timestamp ? new Date(message.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}</span>
            </div>
          )}

          {/* ===== THINKING PANEL — DeepSeek Glass Box ===== */}
            ...
            <div className="thinking-panel">
              ...
              >
                <div className="thinking-header-left">
                  <div className={`thinking-icon ${isThinking ? 'active' : ''}`}>
                    <Brain className="w-3.5 h-3.5" />
                  </div>
                  {isThinking ? (
                    <>
                      <span className="text-accent font-semibold">Pensando</span>
                ...
                <ChevronDown className={`thinking-chevron ${isReasoningOpen ? 'open' : ''}`} />
              </button>
              {isReasoningOpen && (
                <div className="thinking-body">
                  {message.reasoning_content ? (
                    ...
                    <div className="space-y-2 font-sans" aria-label="Razonando">
            ...
            <div
              className={`${
                isUser
                  ...
                  : 'text-sm sm:text-[15px] text-neutral-800 dark:text-neutral-200 leading-relaxed break-words max-w-none'
              }`}
            >
              {message.content ? (
                ...
                <div className="text-sm text-neutral-500 dark:text-neutral-400 italic space-y-2 py-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      {message.reasoning_content
                        ? 'El modelo completo el razonamiento pero agoto el limite de tokens antes de generar la respuesta.'
          ...
          {isTruncated && !isStreaming && !message.isError && (
            <div className="mt-3 p-3 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200/80 dark:border-amber-800/60 text-amber-800 dark:text-amber-300 text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
              <div className="flex items-center gap-2">
                ...
                <span>
                  La respuesta se corto al alcanzar el limite de tokens ({message.tokens?.completion || message.tokens?.total || 'max'}).
                </span>
              </div>
